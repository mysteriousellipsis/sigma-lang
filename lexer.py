...
kwpattern: str = (
    r"\b(?:" + "|".join(escapedkw) + r")\b"
)  # changed this so that it matches keywords and ids only if they are not part of a larger word

# regex for all potential keywords
...
class Lexer:
    ...
    def __init__(self, text: str) -> None:
        ...
        self.words: List[str] = [
            next(group for group in match if group) for match in pattern.findall(text)
        ]

        self.currword: Optional[str] = (
            self.words[self.pos] if self.pos < len(self.words) else None
        )
    ...
    def isfloat(x: str) -> bool:
        """returns if argument `x` is a float"""
        try:
            s = x
            float(s)
            return True
    ...
    def tokenize(self) -> list[Token]:
        ...
        for word in self.words:
            ...
            # comments
                ...
                word[0] == const.KEYWORDS["COMMENT_OPEN"]
                and word[-1] == const.KEYWORDS["COMMENT_CLOSE"]
            ):
                continue
            # strings
            elif (
                word[0] in const.KEYWORDS["STRING_DELIMS"]
                and word[-1] in const.KEYWORDS["STRING_DELIMS"]
                and word[0] == word[-1]
            ):
            ...
            # other keywords
                ...
                continue
            # identifiers (variables, floating values)
            else:
